...
public class RequestService
{
		...
		implements Runnable
	{
		...
		public void run()
		{
			IdentifiableObject 	source  = null;
			IdentifiableObject 	target  = null;
			IdentifiableObject  handler = null;
			
			String sourceId 	= request.getAttribute("source-id");
			String sourceType	= request.getAttribute("source-type");
			String targetId 	= request.getAttribute("target-id");
			String targetType	= request.getAttribute("target-type");
			
			if( ! Agency.getLocalAgency().getId().equals(request.getSource()) )
			{
				source = new RemoteIdentifiableObject(request.getSource(),
					sourceId,IdentifiableType.valueOf(sourceType));
			}
			else
			{
				source = Agency.getLocalAgency().getIdentifiableObject(
						IdentifiableType.valueOf(sourceType),sourceId);
				
				if( source == null )
				{
					System.err.printf("[request-service] error, unknown source: %s/%s%n", sourceId, sourceType);
					return;
				}
			}
			...
			if( target == null )
			{
				//System.out.printf("[requests] (%s) %s/%s --> %s/%s%n", request.getName(), sourceId, sourceType, targetId, targetType );
				System.err.printf("[request-service] unknown target: %s%n", targetId);
				return;
			}
			
			if( handlers.containsKey(request.getName()) )
				handler = handlers.get(request.getName());
			else
				handler = target;
			...
			if( handler != null )
			{
				...
				handler.handleRequest(source,target,request);
			}
			else
			{
				System.err.printf("[request-service] no handler to %s%n",request.getName());
			}
		}
	}
	
	private class RequestServiceThreadFactory
		implements ThreadFactory
	{
		...
		{
			return new Thread(requestServiceThreadGroup,r,
					String.format("request-service-thread%X",idGenerator++));
		}
	}
	...
	public void init( Collection<AgencyListener> listeners, Args args )
	{
		...
		if( args.has("keep_time") )
		{
			...
			else if( s.matches("\\d+ (DAYS|HOURS|MINUTES|SECONDS|MILLISECONDS|MICROSECONDS|NANOSECONDS)") )
			{
				...
				keepTime = unit.convert(Long.parseLong(s.substring(0,s.indexOf(' '))),localUnit);
			}
			else keepTime = 200;
		}
		else keepTime = 200;
		...
		this.pool 		= new ThreadPoolExecutor(coreSize,maxSize,
				keepTime,unit,new LinkedBlockingQueue<Runnable>(),threadFactory);
	}
	...
	public void interceptRequest( IdentifiableObject handler, String name )
	{
		...
		else System.err.printf("[request-service] conflict between handler for %s%n",name);
	}
}
