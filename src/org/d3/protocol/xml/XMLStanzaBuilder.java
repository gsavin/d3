...
{
	private static final Pattern ATTR = Pattern.compile( "(\\w[\\w:[-]\\d]*)='([^']*)'" );
	
	private static final Pattern stanzaSimple = Pattern.compile( "<(\\w+[\\w:[-]\\d]*)\\s*((?:\\s+\\w[\\w:[-]\\d]*='[^']*')*)\\s*/>" );
	private static final Pattern stanzaBegin = Pattern.compile( "<(\\w+[\\w:[-]\\d]*)\\s*((?:\\s+\\w[\\w:[-]\\d]*='[^']*')*)\\s*>" );
	...
	public static void main( String [] args )
	{
		...
		for( String xml : xmls )
		{
			try
			{
				System.out.printf( "[test] %s\t", xml );
				Object o = string2stanza(xml);
				System.out.printf( "[ok]%n%s%n", o );
			}
			catch( XMLParseException e )
			{
				System.out.printf( "[bad xml]%n'%s'%n", e.getMessage() );
			}
		}
		
		String benchXML = "<message from='someone@xmpp-server.org'><content>bonjour, comment ça va ?</content><presence status='online'><show/></presence></message>";
		int count = 1000000;
		
		System.out.printf("[bench] %d times, %d characters%n", count, benchXML.length() );
		System.out.printf("[bench] string2stanza%n" );
		...
		System.out.printf("[bench] average : %f ms%n", ( m2 - m1 ) / (float) count );
